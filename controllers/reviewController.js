...
//     if (product == null || (!product.isAvailable && !isAdmin(req))) {
...
//     });
//   } catch (error) {
//     // if unique index triggers (duplicate review), return a friendly message
...
//   // admin only, paginated like orders
...
//   try {
//     // show latest approved reviews; tweak limit if you want
...
//     const existing = await Review.findOne({ productId: productId, email: req.user.email });
//     if (existing != null) {
//       res.status(400).json({ message: "You have already reviewed this product" });
//       return;
//     }
...
//     const reviews = await Review.find({
//       productId: productId,
//       isApproved: true, // public only approved
//     }).sort({ date: -1 });
...
//     res.json({ reviews: reviews, totalPages: totalPages });
//   } catch (error) {
//     console.error("Error fetching all reviews:", error);
//     res.status(500).json({ message: "Failed to fetch reviews" });
//   }
...
//     }
//     const productId = req.params.productId;
//     const review = await Review.findOne({ productId: productId, email: req.user.email });
//     res.json(review); // can be null (no review yet)
//   } catch (e) {
...
//       { productId: productId, email: req.user.email },
//       { 
//         ...(rating != null ? { rating } : {}),
...
//     }
//     const productId = req.params.productId;
//     const result = await Review.deleteOne({ productId: productId, email: req.user.email });
